...
Class WordPress_Readme_Parser {
	...
	function parse_readme_contents( $file_contents ) {
		...
		$remaining_content = '';
		$aRemainingContents = array();
		foreach ( $sections as $s_name => $s_data ) {
			$remaining_content .= "\n<h3>{$s_data['title']}</h3>\n{$s_data['content']}";
			$aRemainingContents[ $s_data['title'] ] = $s_data['content'];
		}
		...
		$r = array(
			...
			'remaining_contents' => $aRemainingContents,
			'upgrade_notice' => isset( $upgrade_notice ) ? $upgrade_notice : null,
		);
